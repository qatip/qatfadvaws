...
variable "app_subnet_cidr" {
  ...
  validation {
    condition     = can(cidrhost(trimspace(var.app_subnet_cidr), 0))
    error_message = "app_subnet_cidr must be a valid IPv4 CIDR value."
  }
}
...
variable "data_subnet_cidr" {
  ...
  validation {
    condition     = can(cidrhost(trimspace(var.data_subnet_cidr), 0))
    error_message = "data_subnet_cidr must be a valid IPv4 CIDR value."
  }
}
